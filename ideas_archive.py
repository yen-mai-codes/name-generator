'''Page for managing ideas'''

import streamlit as st
import pandas as pd
from utils.utils import (
    manage_word,
    get_file,
    write_to_file
)

from pinterest.organic.boards import Board

idea_components_file = 'data/idea_components.json'
idea_components = get_file(idea_components_file)['components']

st.markdown('# Ideas Components Archive 📘')

st.write(type(Board))
st.write(Board.get_all())


# def on_submit(action: str):    
#     """
#     Callback function when user submits word inputs.
#     """

#     # Change words json based on user's input action and word
#     new_idea_components = manage_word(action, st.session_state.idea, st.session_state.type, idea_components)

#     # Write to words file the new words json
#     write_to_file(idea_components_file, new_idea_components)



# # ---Side bar
# # Word Input
# col1, col2 = st.columns(2)
# st.markdown('# Inputs')
# col1, col2 = st.columns([2,1], vertical_alignment='center')
# with col1:
#     st.text_input('Word', key='word', label_visibility='collapsed', placeholder='Enter cool word')
#     st.multiselect('POS', pos_types, key='pos', label_visibility='collapsed', placeholder='Choose POS tags')

# with col2:
#     st.button('Add', on_click=on_submit, args=('Add 💏',), use_container_width=True)
#     st.button('Remove', on_click=on_submit, args=('Remove 🤮',), use_container_width=True)

# # Main
# # Word list
# words_df = pd.DataFrame.from_records(words['words'], index='word').sort_values(by=['word'])
# st.table(words_df)